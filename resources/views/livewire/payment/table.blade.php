<div>
    ...
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-extrabold text-slate-900 dark:text-white">Payments</h1>
            <p class="mt-2 text-lg text-slate-500 dark:text-slate-400">Manage student fees and payments</p>
        </div>
        ...
        <table class="w-full text-left text-sm">
            <thead class="bg-slate-50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-300">
                <tr class="border-b border-slate-200 dark:border-slate-800 font-extrabold">
                    <th class="px-7 py-5">Receipt No</th>
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-slate-800 text-slate-800 dark:text-slate-200">
                @forelse ($payments as $payment)
                    <tr class="transition hover:bg-slate-50 dark:hover:bg-slate-800/50">
                        <td class="px-7 py-5">
                            <p class="font-bold text-slate-900 dark:text-white">{{ $payment->receipt_no }}</p>
                            <p class="text-xs text-slate-500 dark:text-slate-400">{{ $payment->payment_method }}</p>
                        </td>
                        <td class="px-7 py-5">
                            <p class="font-bold text-slate-900 dark:text-white">{{ $payment->student->name }}</p>
                            <p class="text-slate-500 dark:text-slate-400">{{ $payment->student->student_id }}</p>
                        </td>
                        ...
                        <td class="px-7 py-5">
                            @php
                                $statusClass = match ($payment->status) {
                                    'Paid' => 'bg-green-100 text-green-700 dark:bg-green-900/50 dark:text-green-400 border border-green-200 dark:border-green-800',
                                    'Pending' => 'bg-amber-100 text-amber-700 dark:bg-amber-900/50 dark:text-amber-400 border border-amber-200 dark:border-amber-800',
                                    'Overdue' => 'bg-red-100 text-red-700 dark:bg-red-900/50 dark:text-red-400 border border-red-200 dark:border-red-800',
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-7 py-16 text-center">
                            <p class="text-lg font-bold text-slate-700 dark:text-slate-300">No payments found</p>
